...
func (u *UserRepositoryImplement) SelectUser(id int) ([]models.User, error) {
	users := make([]models.User, 0)
	rows, error := u.Db.Query("SELECT * FROM users WHERE ID = $1", id)
	fmt.Println(error)
	if error != nil {
		return users, error
	}
...
func (u *UserRepositoryImplement) DeleteUser(id int) error {
	_, error := u.Db.Query("DELETE FROM users WHERE ID = $1", id)
	fmt.Println(error)
	return error
}
